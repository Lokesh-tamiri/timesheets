...
const CreateTimesheet = () => {
  ...
  const VALIDATION_SCHEMA = Yup.object().shape({
    ...
    hours_spent:Yup.number("Please enter only numbers").required("Hours Spent Id is required").max(12,"Hours Spent should not be greater than 12"),
    task_completed:Yup.string().required("Task Completed is required").max(200,"Should not exceed more than 200 Characters"),
    task_pending:Yup.string().required("Task pending is required").max(200,"Should not exceed more than 200 Characters"),
      ...
      <Modal open={open} onClose={handleClose}>
        <Box sx={{...styles.modalStyle,py:4}}>
          <Formik onSubmit={(values)=>{
            console.log(values)
          }} initialValues={INITIAL_VALUES} validationSchema={VALIDATION_SCHEMA}>
            <Form className='flex flex-col gap-5 max-h-[500px] overflow-y-auto px-5 py-5'>
              ...
              <button className={`${styles.primaryBtn} mx-auto`} type='submit'>Create Timesheet</button>
            </Form>
          </Formik>
        </Box>
      </Modal>
